...
export class CreateJob {
  ...
  async execute(jobData) {
    if (!jobData.title?.trim())       throw new Error('El título de la vacante es obligatorio.');
    if (!jobData.location?.trim())    throw new Error('La ubicación es obligatoria.');
    if (!jobData.description?.trim()) throw new Error('La descripción es obligatoria.');
